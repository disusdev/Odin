...
// Get proxy user data
@(require_results)
DynamicTree_GetUserData :: proc "c" (tree: ^DynamicTree, proxyId: c.int) -> u64 {
	return tree.nodes[proxyId].userData
}

// Get the AABB of a proxy
@(require_results)
DynamicTree_GetAABB :: proc "c" (tree: ^DynamicTree, proxyId: c.int) -> AABB {
...
@(require_results)
GetHullVertices :: proc "c" (hull: ^HullData) -> [^]HullVertex {
	if hull.vertexOffset == 0 {
		return nil
	}
	return cast([^]HullVertex)(uintptr(hull) + uintptr(hull.vertexOffset))
}

// Get read only hull points.
@(require_results)
GetHullPoints :: proc "c" (hull: ^HullData) -> [^]Vec3 {
	if hull.pointOffset == 0 {
		return nil
	}
	return cast([^]Vec3)(uintptr(hull) + uintptr(hull.pointOffset))
}

// Get read only hull half edges.
@(require_results)
GetHullEdges :: proc "c" (hull: ^HullData) -> [^]HullHalfEdge {
	if hull.edgeOffset == 0 {
		return nil
	}
	return cast([^]HullHalfEdge)(uintptr(hull) + uintptr(hull.edgeOffset))
}

// Get read only hull faces.
@(require_results)
GetHullFaces :: proc "c" (hull: ^HullData) -> [^]HullFace {
	if hull.faceOffset == 0 {
		return nil
	}
	return cast([^]HullFace)(uintptr(hull) + uintptr(hull.faceOffset))
}

// Get read only hull planes.
@(require_results)
GetHullPlanes :: proc "c" (hull: ^HullData) -> [^]Plane {
	if hull.planeOffset == 0 {
		return nil
	}
	return cast([^]Plane)(uintptr(hull) + uintptr(hull.planeOffset))
}
...
@(require_results)
GetMeshNodes :: proc "c" (mesh: ^MeshData) -> [^]MeshNode {
	if mesh.nodeOffset == 0 {
		return nil
	}
	return cast([^]MeshNode)(uintptr(mesh) + uintptr(mesh.nodeOffset))
}

// Get read only mesh vertices.
@(require_results)
GetMeshVertices :: proc "c" (mesh: ^MeshData) -> [^]Vec3 {
	if mesh.vertexOffset == 0 {
		return nil
	}
	return cast([^]Vec3)(uintptr(mesh) + uintptr(mesh.vertexOffset))
}

// Get read only mesh triangles.
@(require_results)
GetMeshTriangles :: proc "c" (mesh: ^MeshData) -> [^]MeshTriangle {
	if mesh.triangleOffset == 0 {
		return nil
	}
	return cast([^]MeshTriangle)(uintptr(mesh) + uintptr(mesh.triangleOffset))
}

// Get read only mesh materials. The count is equal to the triangle count.
@(require_results)
GetMeshMaterialIndices :: proc "c" (mesh: ^MeshData) -> [^]u8 {
	if mesh.materialOffset == 0 {
		return nil
	}
	return cast([^]u8)(uintptr(mesh) + uintptr(mesh.materialOffset))
}
...
GetMeshFlags :: proc "c" (mesh: ^MeshData) -> [^]u8 {
	...
	return cast([^]u8)(uintptr(mesh) + uintptr(mesh.flagsOffset))
}
...
GetHeightFieldCompressedHeights :: proc "c" (hf: ^HeightFieldData) -> [^]u16 {
	...
	return cast([^]u16)(uintptr(hf) + uintptr(hf.heightsOffset))
}
...
GetHeightFieldMaterialIndices :: proc "c" (hf: ^HeightFieldData) -> [^]u8 {
	...
	return cast([^]u8)(uintptr(hf) + uintptr(hf.materialOffset))
}
...
GetHeightFieldFlags :: proc "c" (hf: ^HeightFieldData) -> [^]u8 {
	...
	return cast([^]u8)(uintptr(hf) + uintptr(hf.flagsOffset))
}
...
foreign lib {
	...
	QueryCompound :: proc(compound: ^CompoundData, aabb: AABB, fcn: CompoundQueryFcn, ctx: rawptr) ---
...
// Query callback.
MeshQueryFcn :: #type proc "c" (a, b, c: Vec3, triangleIndex: i32, ctx: rawptr) -> bool
...
foreign lib {
	...
	OverlapCompound :: proc(shape: ^CompoundData, shapeTransform: Transform, #by_ptr proxy: ShapeProxy) -> bool ---

	// Overlap shape versus height field
	OverlapHeightField :: proc(shape: ^HeightFieldData, shapeTransform: Transform, #by_ptr proxy: ShapeProxy) -> bool ---

	// Overlap shape versus hull
	OverlapHull :: proc(shape: ^HullData, shapeTransform: Transform, #by_ptr proxy: ShapeProxy) -> bool ---
	...
	RayCastCompound :: proc(shape: ^CompoundData, #by_ptr input: RayCastInput) -> CastOutput ---

	// Ray cast versus hull shape in local space. A zero length ray is a point query. Initial overlap
	// reports a hit at the ray origin with zero fraction and zero normal.
	RayCastHull :: proc(shape: ^HullData, #by_ptr input: RayCastInput) -> CastOutput ---
	...
	RayCastHeightField :: proc(shape: ^HeightFieldData, #by_ptr input: RayCastInput) -> CastOutput ---
	...
	ShapeCastCompound :: proc(shape: ^CompoundData, #by_ptr input: ShapeCastInput) -> CastOutput ---

	// Shape cast versus a hull. Initial overlap is treated as a miss.
	ShapeCastHull :: proc(shape: ^HullData, #by_ptr input: ShapeCastInput) -> CastOutput ---
	...
	ShapeCastHeightField :: proc(shape: ^HeightFieldData, #by_ptr input: ShapeCastInput) -> CastOutput ---
...
foreign lib {
	...
	// @param fcn a user function to collect triangles
	// @param context the context sent to the user function.
	QueryHeightField :: proc(heightField: ^HeightFieldData, bounds: AABB, fcn: MeshQueryFcn, ctx: rawptr) ---
}
...
foreign lib {
	// Collide two spheres.
	CollideSpheres :: proc(manifold: ^LocalManifold, capacity: c.int, #by_ptr sphereA: Sphere, #by_ptr sphereB: Sphere, transformBtoA: Transform) ---
	...
	CollideHullAndSphere :: proc(manifold: ^LocalManifold, capacity: c.int, hullA: ^HullData, #by_ptr sphereB: Sphere, transformBtoA: Transform, cache: ^SimplexCache) ---

	// Collide two capsules.
	CollideCapsules :: proc(manifold: ^LocalManifold, capacity: c.int, #by_ptr capsuleA: Capsule, #by_ptr capsuleB: Capsule, transformBtoA: Transform) ---

	// Collide a hull and a capsule.
	CollideHullAndCapsule :: proc(manifold: ^LocalManifold, capacity: c.int, hullA: ^HullData, #by_ptr capsuleB: Capsule, transformBtoA: Transform, cache: ^SimplexCache) ---
	...
	CollideCapsuleAndTriangle :: proc(manifold: ^LocalManifold, capacity: c.int, #by_ptr capsuleA: Capsule, triangleB: [^]Vec3, cache: ^SimplexCache) ---
	...
	CollideSphereAndTriangle :: proc(manifold: ^LocalManifold, capacity: c.int, #by_ptr sphereA: Sphere, triangleB: [^]Vec3) ---
}
